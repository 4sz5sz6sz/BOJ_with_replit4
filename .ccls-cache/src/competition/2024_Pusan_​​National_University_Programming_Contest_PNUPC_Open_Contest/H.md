...
입력
첫 번째 줄에 정점의 개수 $N$과 활성화된 간선의 개수 $M$, 질의의 개수 $Q$가 공백으로 구분되어 주어진다. 

$(1 \leq N \leq 10^5; 0 \leq M \leq \min(\frac{N(N-1)}{2}, 2 \times 10^5); 1 \leq Q \leq 10^5)$ 
...
위의 세 그림은 각각 예제 $1$의 그래프 $G$와 $G$에 첫 번째 질의를 시행한 그래프, $G$에 두 번째 질의를 시행한 그래프이다. 파란색으로 색칠된 간선이 활성화된 간선, 빨간색으로 색칠된 간선이 비활성화된 간선이다.
